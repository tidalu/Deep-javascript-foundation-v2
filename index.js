...
/**
    num++; will return the same num , but after this line if we call num again, it will be incremented 

    ++num; it will return the incremented in place num
    ...
    why this happens cuz ++ increment sign has its own algorithm that first coerce the input to Number then increment and the first time it returns the number form of it next time if we call that it will be incremented;
    ...
    1, bigint is used to store big intizer

    const a = 20;
    console.log(BigInt(a)); // first way
...
==========================================
        ...
        // undefined // it is absolutely equal to itself 
        // undeclared 

        undefined -> it means there is absolutely a variable without initializing but there is one value implicitly on that which is undefined
        // undeclared which has never been declared in any scope,

    ==========================================

    // NaN -> everyone says it is Not a number butt it is totally wrong Nan indicates invalid number
    ...
    // NaN's are not equal to each other cuz their base are different even f their bases are the same but that does not means they are equal , cuz nan is not equal to nan

    myCatsAge === myCatsAge; // false b (===) It tells you that two things are not equal.
    ...
    let trip_x = 0 === zero
    console.log(trip_x); // true

    console.log(zero.toString()); anytime if we convert negative number to string all negative signs will be removed
    ...
    Object.is works correctly for this 


    Math.sign(-0); // -0
    Math.sign(0); 0 that is corner case
